...
bool LockStore::Transaction::operator==(const Transaction &t) { return id == t.id; }

LockStore::LockStore() : store() {}
LockStore::~LockStore() {}

// Gets the running transaction. Creates one if there isn't one for this client
LockStore::Transaction &LockStore::getTxn(uint64_t id) {
...
LockStore::Transaction LockStore::getRetiredTxn(opnum_t op, uint64_t id, RetiredState state) {
 ...
 */
void LockStore::dropLocks(const Transaction &txn) {
 ...
 */
void LockStore::getLocks(const Transaction &txn) {
    bool ret;
    for (auto &write : txn.writeSet) {
...
void LockStore::begin(uint64_t id) {
    Debug("[%" PRIu64 "] BEGIN", id);
    Transaction txn(id);
    running[id] = txn;
}

void LockStore::unbegin(uint64_t id) {
...
int LockStore::get(uint64_t id, const string &key, string &value) {
...
void LockStore::unget(uint64_t id, const string &key) {
...
int LockStore::put(uint64_t id, const string &key, const string &value) {
...
void LockStore::unput(uint64_t id, const string &key, const string &value) {
...
int LockStore::prepare(uint64_t id, opnum_t op) {
...
void LockStore::unprepare(uint64_t id, opnum_t op) {
...
void LockStore::commit(uint64_t id, uint64_t timestamp, opnum_t op) {
...
void LockStore::uncommit(uint64_t id, uint64_t timestamp, opnum_t op) {
...
void LockStore::abortTxn(uint64_t id, opnum_t op) {
...
void LockStore::unabort(uint64_t id, opnum_t op) {
...
void LockStore::specCommit(opnum_t op) {
