...
bool OCCStore::Transaction::operator==(const Transaction &t) { return id == t.id; }

OCCStore::OCCStore() : store() {}
OCCStore::~OCCStore() {}

set<string> OCCStore::getPreparedWrites() {
...
set<string> OCCStore::getPreparedReadWrites() {
    // gather up the set of all writes that we are currently prepared for
...
OCCStore::Transaction &OCCStore::getTxn(uint64_t id) {
...
OCCStore::Transaction OCCStore::getRetiredTxn(opnum_t op, uint64_t id, RetiredState state) {
...
void OCCStore::begin(uint64_t id) {
    Debug("[%" PRIu64 "] BEGIN", id);
    Transaction txn(id);
    running[id] = txn;
}

void OCCStore::unbegin(uint64_t id) {
...
int OCCStore::get(uint64_t id, const string &key, string &value) {
...
void OCCStore::unget(uint64_t id, const string &key) {
...
int OCCStore::put(uint64_t id, const string &key, const string &value) {
...
void OCCStore::unput(uint64_t id, const string &key, const string &value) {
...
int OCCStore::prepare(uint64_t id, opnum_t op) {
    ...
    if (running.find(id) == running.end()) {
        ...
        for (auto read : txn.readSet) {
            ...
            if (cur.first != read.second.first) {
                Debug("[%" PRIu64 "] ABORT rw conflict key:%s", id, read.first.c_str());
            ...
            if (pWrites.find(read.first) != pWrites.end()) {
                Debug("[%" PRIu64 "] ABORT rw conflict w/ prepared key:%s", id, read.first.c_str());
                abortTxn(id, op);
                return -1;
            }
        }
        ...
        for (auto write : txn.writeSet) {
            ...
            if (pRW.find(write.first) != pRW.end()) {
                Debug("[%" PRIu64 "] ABORT ww conflict w/ prepared key:%s", id,
                      write.first.c_str());
                abortTxn(id, op);
                return -1;
            }
...
void OCCStore::unprepare(uint64_t id, opnum_t op) {
...
void OCCStore::commit(uint64_t id, uint64_t timestamp, opnum_t op) {
...
void OCCStore::uncommit(uint64_t id, uint64_t timestamp, opnum_t op) {
...
void OCCStore::abortTxn(uint64_t id, opnum_t op) {
...
void OCCStore::unabort(uint64_t id, opnum_t op) {
...
void OCCStore::specCommit(opnum_t op) {
