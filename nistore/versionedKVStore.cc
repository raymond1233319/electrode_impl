...
bool VersionedKVStore::get(const string &key, pair<uint64_t, string> &value) {
...
bool VersionedKVStore::get(const string &key, uint64_t timestamp, string &value) {
    // check for existence of key in store
    if (store.find(key) == store.end()) {
        return false;
    } else {
...
bool VersionedKVStore::put(const string &key, const string &value, uint64_t timestamp) {
...
bool VersionedKVStore::remove(const string &key, pair<uint64_t, string> &value) {
    if (store.find(key) == store.end()) {
        return false;
    }
