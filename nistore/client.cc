...
Client::Client(Proto mode, string configPath, int nShards) : transport(0.0, 0.0, 0) {
...
void Client::send_begin(unsigned int i) {
    ...
    transport.Timer(0, [=]() {
        shard[i]->Invoke(request_str,
                         bind(&Client::beginCallback, this, i, placeholders::_1, placeholders::_2));
    });
}
...
bool Client::Get(const string &key, string &value) {
    ...
    transport.Timer(0, [=]() {
        shard[i]->Invoke(request_str,
                         bind(&Client::getCallback, this, i, placeholders::_1, placeholders::_2));
    });
...
void Client::Put(const string &key, const string &value) {
    ...
    transport.Timer(0, [=]() {
        shard[i]->Invoke(request_str,
                         bind(&Client::putCallback, this, i, placeholders::_1, placeholders::_2));
    });
...
bool Client::Commit() {
    ...
    // 1. Send commit-prepare to all shards.
    {
        ...
        for (set<int>::iterator it = all_participants.begin(); it != all_participants.end(); it++) {
            Debug("[shard %d] Sending prepare", *it);
            transport.Timer(0, [=]() {
                shard[*it]->Invoke(request_str, bind(&Client::prepareCallback, this, *it,
    ...
    if (nreplies == all_participants.size() && status == true) {
        // Get a timestamp from the tss
        {
            ...
            transport.Timer(0, [=]() {
                tss->Invoke("",
                            bind(&Client::tssCallback, this, placeholders::_1, placeholders::_2));
            });
        ...
        // Send Commit to all shards
        {
            ...
            nreplies = 0;
            for (set<int>::iterator it = all_participants.begin(); it != all_participants.end();
                 it++) {
                Debug("[shard %d] Sending commit", *it);
                transport.Timer(0, [=]() {
                    shard[*it]->Invoke(request_str, bind(&Client::commitCallback, this, *it,
                                                         placeholders::_1, placeholders::_2));
                });
            }
...
void Client::Abort() {
    ...
    for (set<int>::iterator it = all_participants.begin(); it != all_participants.end(); it++) {
        if (yes_participants.find(*it) != yes_participants.end()) {
            ...
            transport.Timer(0, [=]() {
                shard[*it]->Invoke(request_str, bind(&Client::abortCallback, this, *it,
                                                     placeholders::_1, placeholders::_2));
            });
        }
    }
...
void Client::beginCallback(const int index, const string &request_str, const string &reply_str) {
...
void Client::getCallback(const int index, const string &request_str, const string &reply_str) {
    lock_guard<mutex> lock(cv_m);
...
void Client::putCallback(const int index, const string &request_str, const string &reply_str) {
    lock_guard<mutex> lock(cv_m);
...
void Client::prepareCallback(const int index, const string &request_str, const string &reply_str) {
    lock_guard<mutex> lock(cv_m);
...
void Client::commitCallback(const int index, const string &request_str, const string &reply_str) {
    lock_guard<mutex> lock(cv_m);
...
void Client::abortCallback(const int index, const string &request_str, const string &reply_str) {
    lock_guard<mutex> lock(cv_m);
...
long Client::key_to_shard(const string &key) {
