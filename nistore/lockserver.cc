...
bool LockServer::lockForRead(const string &lock, uint64_t requester) {
    Lock &l = locks[lock];
    Debug("Lock for Read: %s [%" PRIu64 " %" PRIu64 " %lu %lu]", lock.c_str(), readers, writers,
    ...
    case LOCKED_FOR_READ:
        ...
        if (l.isWriteNext()) {
            Debug("[%" PRIu64 "] Waiting on lock because there is a pending write request",
                  requester);
...
bool LockServer::lockForWrite(const string &lock, uint64_t requester) {
    ...
    Debug("Lock for Write: %s [%" PRIu64 " %" PRIu64 " %lu %lu]", lock.c_str(), readers, writers,
          l.holders.size(), l.waiters.size());
    ...
    case LOCKED_FOR_READ:
        ...
        Debug("Locked for read by%s%lu other people", l.holders.count(requester) > 0 ? "you" : "",
              l.holders.size());
    ...
    case LOCKED_FOR_READ_WRITE:
        ...
        Debug("Held by %" PRIu64 " for %s", *(l.holders.begin()),
