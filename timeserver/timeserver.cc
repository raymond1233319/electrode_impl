...
void TimeStampServer::ReplicaUpcall(opnum_t opnum, const string &str1, string &str2) {
...
void TimeStampServer::RollbackUpcall(opnum_t current, opnum_t to,
...
/* Ignore for now, will be used when running specpaxos. */
void TimeStampServer::CommitUpcall(opnum_t commitOpnum) {
...
int main(int argc, char **argv) {
    ...
    enum { PROTO_UNKNOWN, PROTO_VR, PROTO_SPEC, PROTO_FAST } proto = PROTO_UNKNOWN;
    ...
    case PROTO_FAST:
        replica =
            new specpaxos::fastpaxos::FastPaxosReplica(config, index, true, &transport, &server);
        break;
